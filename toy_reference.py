...
# data
folder_root = "./data/" # where the data are stored                                                                     
...
#optimizer1 = Adam()
#optimizer2 = Adam()
#optimizer3 = Adam()#tf.keras.optimizers.Adam()
t1 =time.time()
...
if train_coeffs:
    ...
    alternate_training["train_coeffs"]["train_vars"]=model.trainable_variables[0:1]#[0+1*(train_centroids==True)+1*(train_widths==True):1+1*(train_centroids==True)+1*(train_widths==True)]#model.coeffs#
    ...
    alternate_training["train_coeffs"]['opt'] = Adam(learning_rate=1e-1)

if train_widths:
    alternate_training["train_widths"] = {}
    alternate_training["train_widths"]["train_vars"]=model.trainable_variables[0+1*(train_centroids==True)+1*(train_widths==True):1+1*(train_centroids==True)+1*(train_widths==True)]#model.kernel_layer.widths#
    ...
    alternate_training["train_widths"]['opt'] = Adam()
    
if train_centroids:
    alternate_training["train_centroids"] = {}
    alternate_training["train_centroids"]["train_vars"]=model.trainable_variables[0+1*(train_centroids==True):1+1*(train_centroids==True)]#model.kernel_layer.centroids#
    ...
    alternate_training["train_centroids"]['opt'] = Adam(learning_rate=1e-1)
...
for i in range(int(total_epochs)):
    for train_step in list(alternate_training.keys()):
        ...
        for j in range(sub_epochs):
            ...
            optimizer.apply_gradients(grads,train_vars)#zip([grads],[train_vars]))
            #model.clip_centroids()
            model.clip_coeffs()
        if not (i%patience):
