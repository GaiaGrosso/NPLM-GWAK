...
if M<=ND:
    idx = np.random.randint(len(featureData), size=ND)
...
if train_coeffs:
    ...
    alternate_training["train_coeffs"]["train_vars"]=model.trainable_variables[1+1*(train_widths==True):2+1*(train_widths==True)]
...
if train_widths:
    ...
    alternate_training["train_widths"]["train_vars"]=model.trainable_variables[0:1]
...
if train_centroids:
    ...
    alternate_training["train_centroids"]["train_vars"]=model.trainable_variables[0+1*(train_widths==True):1+1*(train_widths==True)]
...
for i in range(int(total_epochs)):
    ...
    w_dat = target[:, 1]
    ref_preds = model(feature[target[:, 0]==0])
