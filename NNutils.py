...
class KernelMethod(Model):
    ...
    def get_centroids_entropy(self):
        return tf.cast(self.kernel_layer.get_centroids_entropy(), dtype=tf.double)
    
    def get_coeffs(self):
        return tf.cast(self.coeffs, dtype=tf.double)

    def get_centroids(self):
        return tf.cast(self.kernel_layer.get_centroids(), dtype=tf.double)

    def get_widths(self):
        return tf.cast(self.kernel_layer.get_widths(), dtype=tf.double)

    def get_widths_tilde(self):
        return tf.cast(self.kernel_layer.get_widths_tilde(), dtype=tf.double)
...
#        log_K_mu = tf.reduce_sum(log_K_mu, axis=1) # [M,]
    ...
    def gauss_const(self, widths):
        ...
        det_sigma_sq = tf.reduce_sum(cov_diag, axis=1) # [M,]                                                     
        return 1./(tf.cast(tf.sqrt(2*np.pi), dtype=tf.float64)**cov_diag.shape[1])/tf.sqrt(det_sigma_sq)
    ...
    def Kernel(self, consts, widths, x):
        ...
        """
        cov_diag = tf.multiply(widths, widths) #[M, d]                                                            
        dist_sq  = tf.math.subtract(tf.expand_dims(x,axis=1),tf.expand_dims(self.centroids,axis=0))**2 # [N, M, d]                                                        
