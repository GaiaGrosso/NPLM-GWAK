...
coeffs_init = torch.from_numpy(coeffs_init).double()
widths_init = torch.from_numpy(widths_init).double()
centroids_init = torch.from_numpy(centroids_init).double()

resolution_const=torch.from_numpy(resolution_const).double()
resolution_scale=torch.from_numpy(resolution_scale).double()

##### define output path ######################
OUTPUT_PATH    = config_json["output_directory"]+'/signals/'
...
print("coeff", L2Regularizer(model.get_coeffs()).detach().numpy())
...
widths_history    = model.get_widths().detach().numpy().reshape((1, M, -1))
centroids_history = model.get_centroids().detach().numpy().reshape((1, M, -1))
coeffs_history    = model.get_coeffs().detach().numpy().reshape((1, M))
nplm_loss_history = np.array([nplm_loss_value.detach().numpy()])
epochs_history    = np.array([0])
loss_history      = np.array([loss_value.detach().numpy()])

# define alternate training
alternate_training={}
if train_coeffs:
    alternate_training["train_coeffs"] = {}
    alternate_training["train_coeffs"]["train_vars"]=model.get_coeffs()
    ...
    alternate_training["train_coeffs"]['opt'] =  torch.optim.Adam([model.get_coeffs()], lr=1e-1)

if train_widths:
    alternate_training["train_widths"] = {}
    alternate_training["train_widths"]["train_vars"]=model.get_widths()
    ...
    alternate_training["train_widths"]['opt'] = torch.optim.Adam([model.get_widths()])

if train_centroids:
    alternate_training["train_centroids"] = {}
    alternate_training["train_centroids"]["train_vars"]=model.get_centroids()
    ...
    alternate_training["train_centroids"]['opt'] = torch.optim.Adam([model.get_centroids()], lr=1e-1)
...
for i in range(int(total_epochs)):
    for train_step in list(alternate_training.keys()):
        ...
        for j in range(sub_epochs):
            ...
            if alternate_training[train_step]["reg_coeffs"]:
                ...
                loss_value = loss_value + reg_coeffs
            if alternate_training[train_step]["reg_widths"]:
                ...
                loss_value = loss_value + reg_widths
            if alternate_training[train_step]["reg_centr"]:
                ...
                loss_value = loss_value + reg_entr
        ...
        if not (i%patience):
            widths_history    = np.concatenate((widths_history, 
                                                model.get_widths().detach().numpy().reshape((1, M, -1))), axis=0)
            centroids_history = np.concatenate((centroids_history, 
                                                model.get_centroids().detach().numpy().reshape((1, M, -1))), axis=0)
            coeffs_history    = np.concatenate((coeffs_history, 
                                                model.get_coeffs().detach().numpy().reshape(1, M)), axis=0)
            nplm_loss_history = np.append(nplm_loss_history, np.array([nplm_loss_value.detach()]))
            loss_history   = np.append(loss_history, loss_value.detach().numpy())
            epochs_history = np.append(epochs_history, i+1)
            print('epoch: %i, loss: %f, NPLM_x: %f, COEFFS: %f, WIDTHS: %f, ENTROPY: %f'%(int(i+1), loss_value,
                                                                                          nplm_loss_value,
                                                                                          reg_coeffs, reg_widths,
                                                                                          reg_entr))
            coeffs_final    = coeffs_history[-1, :]
            widths_final    = widths_history[-1, :, :]
            centroids_final = centroids_history[-1, :, :]
        if np.isnan(loss_value.detach().numpy()): break
    ...
    for	k in range(1):
        ...
        ax= fig.add_axes([0.15, 0.1, 0.78, 0.8])
        for m in range(M):
            plt.plot(epochs_history, centroids_history[:, m, k:k+1], label='%i'%(m))
    ...
    for k in range(1):
        ...
        ax= fig.add_axes([0.15, 0.1, 0.78, 0.8])
        for m in range(M):
            plt.plot(epochs_history, widths_history[:, m, k:k+1], label='%i'%(m))
        font=font_manager.FontProperties(family='serif', size=14)
        plt.ylabel('Widths', fontsize=18, fontname='serif')
        plt.xlabel('Epochs', fontsize=18, fontname='serif')
        plt.xticks(fontsize=16, fontname='serif')
        plt.yticks(fontsize=16, fontname='serif')
        plt.grid()
        plt.show()
        plt.savefig(output_folder+'widths%i.pdf'%(k))
        plt.close()
    
    fig = plt.figure(figsize=(9,6))
    fig.patch.set_facecolor('white')
    ax= fig.add_axes([0.15, 0.1, 0.78, 0.8])
    pred_norm = torch.sigmoid(pred[:, 0]).detach().numpy()
    w_norm = target[:, 1].detach().numpy()
    ...
    plt.close()
    ####
    w_dat = target[:, 1].detach().numpy()
    ref_preds = model.call(feature[target[:, 0]==0])
    dat = feature.detach().numpy()
    centroids_m_final = centroids_history[-1, :, :]#.detach().numpy()
    for k in range(1):
        if k<len(feature):
